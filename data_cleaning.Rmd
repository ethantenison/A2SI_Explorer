...
ej$`Percentile for Ozone level in air` <- as.numeric(ej$`Percentile for Ozone level in air`)
ej$`Percentile for Ozone level in air` <- ej$`Percentile for Ozone level in air`*0.01
ej$`Percentile for PM2.5 level in air`<- as.numeric(ej$`Percentile for PM2.5 level in air`)
ej$`Percentile for PM2.5 level in air` <- ej$`Percentile for PM2.5 level in air`*0.01
...
df$`Average Vehicles per person` <- df$`Aggregate Vehicles` / df$`Total population`

df <- df %>% select(-c("Aggregate Vehicles"))
df$`Average Vehicles per person`[df$`Average Vehicles per person` == 0] <- 0.5
...
df <-
    df %>% 
    select(GEOID_, "Average Vehicles per person", "Percent of households without a car", everything()) %>% 
...
df$var[df$var == "Climate Exposure and Climate Vulnerability"] <- "Climate Exposure and Population Sensitivity"
df$var[df$var == "Composite Climate Hazard Exposure"] <- "Composite Climate Exposure"
df$var[df$var == "Social Vulnerability Score"] <- "Population Sensitivity"
df$var[df$var == "Heat Exposure (Norm"] <- "Heat Exposure (Norm)"
